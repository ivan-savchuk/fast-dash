...
- `variant` (added 2026-08-16) says *how* a chart is drawn — `{ "type": "bar",
  ...
  grouped), **KPI** (with delta, with trend, number only) and **time series** (line,
  area, stacked area); on a table or a section header it would be meaningless, so it is
  absent rather than null. The first listed is
